...
def train_baseline_model(train_df, val_df, max_features=10000):
    print("\n" + "="*60)
...
def evaluate_model(vectorizer, model, test_df, dataset_name):
    ...
    plot_path = os.path.join(RESULTS_DIR, f'baseline_confusion_matrix_{dataset_name.lower().replace(" ", "_")}.png')
...
def save_model(vectorizer, model, model_name='baseline'):
    print("\n" + "="*60)
    print("SAVING MODEL")
    print("="*60)
...
if __name__ == '__main__':
    ...
    vectorizer, model, val_acc = train_baseline_model(
        data['train'], 
        data['val'],
        max_features=10000
    )
    ...
    save_model(vectorizer, model, 'baseline')
